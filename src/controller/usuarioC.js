...
class usuarioC extends controller{
    ...
    crear = async(hash,nom,pat,mat,user,pass,rpass,type)=>{
        try{
            let err = [];
            ...
            if (isAdmin){
                ...
                if (err.length===0) 
                await usuarioM.editar({nombre:nom,apellidoP:pat,apellidoM:mat,user:user,pass:pass,tipoUsuario:1,tipoUsuario:type});
            }
            else
    ...
    borrar = async(hash,idUsuario)=>{
        ...
        let err = [];
        hash = await this.adminCheck() //decifra cookie y verifica si el usuario es admin
        let isAdmin = hash[0];
        hash = hash[1];
        ...
        if (isAdmin){
            ...
            if (hash.idUsuario === idUsuario)
                err.push("Error 403-Un administrador no puede borrarse a si mismo");
            else
            ...
            err.push("Error 403-Solo un administrador puede realizar esta operaciòn");
        return err;
    }
