...
class comiteC extends controller {
    ...
    crear = async (comite) => {
        ...
        //Se eliminan espacios vacios consecutivos
        if (comite != undefined){
            comite = comite.replace(/\s+/g, ' ').trim();

            if (!val.isAlpha(comite.replace(/\s+/g, ' ').trim(), ['es-ES'], { ignore: ' ' }))
            ...
            err.push("Error nombre del comité invalidos");
        return err;
    }
    ...
    editar = async (idComite, comite, miembros) => {
        ...
        if (typeof idComite === "number" && typeof comite === "string" && typeof miembros === "object") {
            if (c.length > 0) { //Si el comite existe
                c = c[0];
                //Validaciòn
                ...
                err.push("Comite no encontrado");
        }
        else
            err.push("Error 500: Datos invalidos");
        return err;
    }
